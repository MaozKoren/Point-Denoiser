...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # add noise
            data[0] = noise.addNoise(data[0], 0)
            ...
            fps_idx = pointnet2_utils.farthest_point_sampling(points, point_all)  # (B, npoint) # changed by Maoz
            fps_idx = fps_idx[:, np.random.choice(point_all, npoints, False)]
            points = gather_new(points.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()  # (B, N, 3) # changed by Maoz
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            fps_idx_raw = pointnet2_utils.farthest_point_sampling(points_raw, point_all)  # (B, npoint)
            local_pred = []

            for kk in range(times):
                fps_idx = fps_idx_raw[:, np.random.choice(point_all, npoints, False)]
                points = gather_new(points_raw.transpose(1, 2).contiguous(), 
...
def test(base_model, test_dataloader, args, config, logger = None):
    ...
    test_label = []
    npoints = config.npoints
    if not config.dataset.train._base_.ADD_NOISE.BOOL:
        with torch.no_grad():
            for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
                points = data[0].cuda()
                label = data[1].cuda()
        ...
        with torch.no_grad():
            for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
                ...
                # dense_points, vis_points, centers = base_model(points)
                dense_points = base_model(points, vis=True)
                print('dense points are:')
                print(dense_points)
                print(f'shape of dense points is: {dense_points.shape} before reshape operation')
                final_image = []
                ...
                print(f'points shape before get_pntcloud_img is: {points.shape}, type is: {points.dtype}')
                # np.savetxt(os.path.join(data_path, 'gt.txt'), points, delimiter=';')
                # points = misc.get_ptcloud_img(points, a, b)
                points = misc.get_ptcloud_img_denoise(points, a, b)
                ...
                print(f'dense points shape before get_pntcloud_img is: {dense_points.shape}, type is: {dense_points.dtype}')
                dense_points = misc.get_ptcloud_img_denoise(dense_points, a, b)
...
def test_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            fps_idx_raw = pointnet2_utils.farthest_point_sampling(points_raw, point_all)  # (B, npoint) # changed by Maoz
            local_pred = []
